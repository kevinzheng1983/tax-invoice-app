...
export default async function PreviewPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  const pdfParams = new URLSearchParams({ number: invoice.number, date: invoice.date, customer: invoice.customer, description: invoice.description, amount: String(invoice.amount), defaultProviderNumber: settings.defaultProviderNumber });
  if (insuranceProvider) {
    pdfParams.set("providerLabel", insuranceProvider.label);
    pdfParams.set("providerNumber", insuranceProvider.number);
  }
  ...
  return <AppShell className="preview-page"><TopBar title="Receipt preview" back="/invoices" action={<DeleteInvoiceButton id={id} number={invoice.number}/>}/>
    ...
    <div className="preview-actions"><Link className="secondary-button" href={`/invoices/${id}/edit`}>Edit</Link><Link className="secondary-button" href={`/invoices/${id}/copy`}>Copy &amp; Edit</Link><SharePdfButton pdfUrl={pdfUrl} receiptNumber={invoice.number} customerName={invoice.customer}/><Link className="secondary-button" href={`/invoices/preview/print?id=${id}`}>View &amp; download PDF</Link></div>
  </AppShell>;
}
