...
export default async function PrintReceiptPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  const params = new URLSearchParams({ number: invoice.number, date: invoice.date, customer: invoice.customer, description: invoice.description, amount: String(invoice.amount), providerLabel: provider.label, providerNumber: provider.number });
